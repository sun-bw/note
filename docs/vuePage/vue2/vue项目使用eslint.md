...
module.exports = {
    ...
    // 指定配置的字符串（配置的路径，可共享配置的名称eslint:recommended 或 eslint:all）
    'extends': [
        'eslint:recommended',
...
```
## 四、安装依赖
<p>npm install --save -dev安装到开发环境</p>
<p>npm install -g 全局安装</p>

## 五、vscode配置eslint
1. 下载eslint插件
2. 安装eslint插件：npm install eslint-plugin-html --save -dev
3. 配置setting文件:文件-首选项-设置，搜索编辑eslint.validate进行编辑，添加内容：
